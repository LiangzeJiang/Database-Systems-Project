...
class FilterReconstructTransposeRule protected (config: RelRule.Config)
  ...
  override def onMatch(call: RelOptRuleCall): Unit = {
    ...
    if (first >= recons.getLeft.getRowType.getFieldCount) {
      ...
      val newfilter = filter.copy(filter.getTraitSet, recons.getRight, newCondition)
      val newrecons = recons.copy(recons.getLeft, newfilter)
      call.transformTo(
        newrecons
      )
      ...
      val newfilter = filter.copy(filter.getTraitSet, recons.getLeft, newCondition)
      val newrecons = recons.copy(newfilter, recons.getRight)
      call.transformTo(
        newrecons
      )
    }
