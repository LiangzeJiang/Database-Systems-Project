...
class Join(
  ...
  override def execute(): IndexedSeq[Column] = {
    ...
    val left_seq = left.toIndexedSeq.transpose.filter(r => r.last==true).map(r => r.dropRight(1))
    val right_seq = right.toIndexedSeq.transpose.filter(r => r.last==true).map(r => r.dropRight(1))
