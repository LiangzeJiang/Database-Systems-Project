...
//    val newagg = agg.copy(agg.getTraitSet, decode.getInput, agg.getGroupSet, agg.groupSets, agg.getAggCallList)
    val newagg = agg.copy(agg.getTraitSet, decode.getInputs)
    val newdecode = decode.copy(newagg)
    call.transformTo(
      newdecode
    )
  }
